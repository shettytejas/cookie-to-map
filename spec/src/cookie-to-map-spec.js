...
describe("CookieToMap's", () => {
  context('#constructor() method', () => {
    it('should always throw an error on call', () => {
      expect(() => new CookieToMap()).to.throw(Error, 'cannot call private constructor.');
    });
  });

  context('#_cookieStringSplitter()', () => {
    it('should split the given string using given regexp delimiter', () => {
      expect(CookieToMap._cookieStringSplitter('this=cookie; string=is_a;test=cookie_; string;', /; ?/)).to.be.eql([
        'this=cookie',
        'string=is_a',
        'test=cookie_',
        'string',
    ...
    it('should return empty array for string constructed using given delimiter', () => {
      expect(CookieToMap._cookieStringSplitter(';; ; ; ;;;; ;; ;   ;  ; ;; ;  ; ; ; ;', /; ?/)).to.be.eql([]);
    });

    it('should trim leading and trailing whitespaces of singular cookie strings (key=value)', () => {
      expect(CookieToMap._cookieStringSplitter('   test=afasfd ,  a=fasdfasf,   afagas=dasdga   ,  ', /, ?/)).to.be.eql(
  ...
  context('#_parseIntoTuple()', () => {
    it('should parse a single cookie string into an array of key-value pair', () => {
      expect(CookieToMap._parseIntoTuple('key=value')).to.be.eql(new CookieTuple('key', 'value'));
    });

    it('should parse a single cookie string with no value into an array of key-value pair', () => {
      expect(CookieToMap._parseIntoTuple('key')).to.be.eql(new CookieTuple('key', undefined));
    });
    ...
    it('should parse cookie string if there are two cookies in the string (delimitted by comma)', () => {
      expect(CookieToMap._parseIntoTuple('language=en,test=value,test2=value2')).to.be.eql([
    ...
    it('should parse cookie string if there are multiple equal to sign in cookie string, but no delimiter', () => {
      expect(CookieToMap._parseIntoTuple('timezone=some_val&=latitude&=longitude&=someotherval')).to.be.eql(
        new CookieTuple('timezone', 'some_val&=latitude&=longitude&=someotherval'),
      );
    });
